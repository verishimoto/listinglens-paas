...
## 5. Cove Administration (Task Governance)
   >
   > **Rule**: The Watcher administers the "Cove" (Project Brain) to ensure no task is left behind.
...
- [ ] **Audit `task.md`**:
  ...
  - Ensure logical flow.
- [ ] **Audit Artifacts**:
  - Check `implementation_plan.md` exists and is up to date.
  - Check `notebooklm_context_injection.md` is fresh.
- [ ] **Report**: If discrepancies are found, fix them immediately or flag them in `task.md`.

## 6. Control Room Watcher
